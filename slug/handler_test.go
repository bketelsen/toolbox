...
func Example() {
	...
	slog.Error("DB connection lost", slug.Err(errors.New("connection reset")), "db", "myapp")
	// Output:
}
...
func TestHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			F: func(l *slog.Logger) {
				l.Error("test", slug.Err(errors.New("fail")))
			},
			Want: `Nov 10 23:00:00.000 ERR test err=fail`,
		},
		{
			F: func(l *slog.Logger) {
				l.Info("test", slog.Group("group", slog.String("key", "val"), slug.Err(errors.New("fail"))))
			},
		...
		{ // https://github.com/lmittmann/tint/pull/66
			F: func(l *slog.Logger) {
				errAttr := slug.Err(errors.New("fail"))
				errAttr.Key = "error"
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			if test.Opts == nil {
				test.Opts = &slug.Options{NoColor: true}
			}
			l := slog.New(slug.NewHandler(&buf, test.Opts))
			test.F(l)
...
func TestReplaceAttr(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			tintLogger := slog.New(slug.NewHandler(io.Discard, &slug.Options{
...
func BenchmarkLogAttrs(b *testing.B) {
	handler := []struct {
		...
		{"tint", slug.NewHandler(io.Discard, nil)},
