...
func Test_DisplayTable(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		expected := `
NAME  ALT NAME  AGE  ROLES      SUB 1 NAME  SUB 1 AGE  SUB 2 NAME  SUB 2 AGE  SUB 3 INNER NAME  SUB 3 INNER AGE  SUB 4       TIME                  TIME PTR              MY STRING
bar   bar alt    20  [a]        bar1               21  <nil>       <nil>      bar3                           23  {bar4 24 }  2022-08-02T15:49:10Z  <nil>                 my string
...
foo   <nil>      10  [a, b, c]  foo1               11  foo2        12         foo3                           13  {foo4 14 }  2022-08-02T15:49:10Z  2022-08-02T15:49:10Z  <nil>
		...
		out, err := ui.DisplayTable(in, "", nil)
		...
		out, err = ui.DisplayTable(inPtr, "", nil)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
...
baz   <nil>      30  []         baz1               31  <nil>       <nil>      baz3                           33  {baz4 34 }  2022-08-02T15:49:10Z  <nil>                 <nil>
		...
		out, err := ui.DisplayTable(in, "age", nil)
		log.Println("rendered table:\n" + out)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
...
foo   foo1        foo3              2022-08-02T15:49:10Z
		...
		out, err := ui.DisplayTable(in, "", []string{"name", "sub_1_name", "sub_3 inner name", "time"})
		log.Println("rendered table:\n" + out)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
...
Alice   25
		...
		out, err := ui.DisplayTable(inlineIn, "", []string{"name", "age"})
		log.Println("rendered table:\n" + out)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
	...
	t.Run("Interfaces", func(t *testing.T) {
		...
		require.NoError(t, err)
		other := []tableTest1{{}}
		expected, err := ui.DisplayTable(other, "", nil)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
...
foo   <nil>      10  [a, b, c]  foo1               11  foo2        12         foo3                           13  {foo4 14 }  2022-08-02T15:49:10Z  2022-08-02T15:49:10Z  <nil>
		...
		for _, v := range in {
			...
			inlineIn = append(inlineIn, ui.TableSeparator{})
		}
		out, err := ui.DisplayTable(inlineIn, "", nil)
		t.Log("rendered table:\n" + out)
		require.NoError(t, err)
		compareTables(t, expected, out)
	})
	...
	t.Run("Errors", func(t *testing.T) {
		...
		t.Run("BadSortColumn", func(t *testing.T) {
			...
			_, err := ui.DisplayTable(in, "bad_column_does_not_exist", nil)
			require.Error(t, err)
		})
		...
		t.Run("BadFilterColumns", func(t *testing.T) {
			...
			_, err := ui.DisplayTable(in, "", []string{"name", "bad_column_does_not_exist"})
			require.Error(t, err)
		})
