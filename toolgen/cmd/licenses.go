...
func getLicense(config *viper.Viper) License {
	...
	if config.IsSet("license.header") || config.IsSet("license.text") {
		return License{Header: config.GetString("license.header"),
			Text: config.GetString("license.text")}
	}

	// If user wants to have built-in license, use that.
	if config.IsSet("license") {
		return findLicense(config.GetString("license"))
	}
...
func copyrightLine(config *viper.Viper) string {
	author := config.GetString("author")
	email := config.GetString("email")

	year := config.GetString("year") // For tests.
	if year == "" {
