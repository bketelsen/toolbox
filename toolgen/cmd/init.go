...
var (
	initCmd = &cobra.Command{
		...
		Short:   "Initialize a toolbox application",
		Long: `Initialize (starter init) will create a new application, with a license
...
`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			projectPath, err := initializeProject(args, cmd.GlobalConfig())
			if err != nil {
			...
			err = goGet("github.com/bketelsen/toolbox/go-version")
			if err != nil {
				cmd.Logger.Error(err.Error())
			}
			cobra.CheckErr(err)
			...
			// }
			fmt.Printf("Your Toolbox application is ready at\n%s\n", projectPath)
		},
	}
)
...
func initializeProject(args []string, config *viper.Viper) (string, error) {
	wd, err := os.Getwd()
	if err != nil {
		return "", err
	}
...
func parseModInfo() (Mod, CurDir) {
	...
	if mod.Path == "command-line-arguments" {
		cobra.CheckErr("Please run `go mod init <MODNAME>` before `toolgen init`")
	}
