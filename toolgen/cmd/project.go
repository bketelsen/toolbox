...
import (
	...
	"github.com/bketelsen/toolbox/toolgen/tpl"
...
func (e *Extras) Create() error {
	if e.Installer {
		...
		e.Project.Config.Set("installer", true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
	...
	if e.Taskfile {
		...
		e.Project.Config.Set(keyTaskfile, true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
			fmt.Println("error writing config file", err)
			return err
	...
	if e.GoReleaser {
		...
		e.Project.Config.Set(keyReleaser, true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
			fmt.Println("error writing config file", err)
			return err
	...
	if e.DevContainer {
		...
		e.Project.Config.Set(keyDevContainer, true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
			fmt.Println("error writing config file", err)
			return err
	...
	if e.ActionsGo {
		...
		e.Project.Config.Set(keyActionsGo, true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
			fmt.Println("error writing config file", err)
			return err
	...
	if e.ActionsPages {
		...
		e.Project.Config.Set(keyActionsPages, true)
		err = e.Project.Config.WriteConfig()
		if err != nil {
			fmt.Println("error writing config file", err)
			return err
...
func (p *Project) createLicenseFile() error {
	data := map[string]interface{}{
		"copyright": copyrightLine(p.Config),
	}
...
func (c *Command) Docs() error {
	...
	fmt.Println("Copying hugo site files")

	err = os.CopyFS(".", tpl.SiteFS)

	if err != nil {
		return err
	}
	// Copy the hugo content stubs
	fmt.Println("Copying hugo content files")

	err = os.CopyFS(".", tpl.ContentFS)

	if err != nil {
		return err
	}
	modFile, err := os.Create(fmt.Sprintf("%s/hugo.mod", c.AbsolutePath))
	if err != nil {
		return err
	}
	defer modFile.Close()
	docModTemplate := template.Must(template.New("docmod").Parse(string(tpl.HugoModTemplate)))
	err = docModTemplate.Execute(modFile, c)
	if err != nil {
		return err
	}
	hugoFile, err := os.Create(fmt.Sprintf("%s/hugo.yaml", c.AbsolutePath))
	if err != nil {
		return err
	}
	defer modFile.Close()
	hugoTemplate := template.Must(template.New("hugocfg").Parse(string(tpl.HugoYamlTemplate)))
	err = hugoTemplate.Execute(hugoFile, c)
	if err != nil {
		return err
	}

	docsTask, err := os.Create(fmt.Sprintf("%s/Taskfile.docs.yml", c.AbsolutePath))
	if err != nil {
		return err
	}
	...
	c.Project.Config.Set(keyDocs, true)
	err = c.Project.Config.WriteConfig()
	if err != nil {
		fmt.Println("error writing config file", err)
		return err
	}
